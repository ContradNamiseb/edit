...
impl<T> MeVec<T> {
    ...
    pub const fn new() -> Self {
        MeVec { ptr: NonNull::dangling(), cap: 0, len: 0, _marker: PhantomData }
    }
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        ...
        MeVec { ptr, cap, len: 0, _marker: PhantomData }
    }
...
impl<T> DerefMut for MeVec<T> {
    fn deref_mut(&mut self) -> &mut [T] {
        unsafe { std::slice::from_raw_parts_mut(self.ptr.as_ptr(), self.len) }
    }
}

impl<T> AsRef<[T]> for MeVec<T> {
    fn as_ref(&self) -> &[T] {
        self.deref()
    }
}

impl<T> AsMut<[T]> for MeVec<T> {
    fn as_mut(&mut self) -> &mut [T] {
        self.deref_mut()
    }
}

impl<T> Borrow<[T]> for MeVec<T> {
    fn borrow(&self) -> &[T] {
        self.deref()
    }
}
